...
const SearchFilters: React.FC<SearchFilterProps> = ({ onSearch, onClose }) => {
    ...
    useEffect(() => {
        const loadCities = async () => {
            if (filters.countryId !== undefined) {
                const data = await locationService.getCitiesByCountry(filters.countryId);
                setCities(data);
            } else {
                setCities([]);
            }
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const params: Partial<WarehouseSearchParams> = {};

        (Object.keys(filters) as Array<keyof WarehouseSearchParams>).forEach(
            (key) => {
