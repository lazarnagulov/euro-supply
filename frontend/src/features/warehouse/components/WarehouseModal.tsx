...
import type { Sector, Warehouse, WarehouseResponse } from "../types/warehouse.type";
import { locationService } from "../../../api/services/locationService";
import { warehouseCreateSchema, warehouseImagesSchema } from "../schemas/warehouseCreateSchemas.ts";
...
import { warehouseEditSchema } from "../schemas/warehouseEditSchema.ts";
...
const WarehouseModal: React.FC<WarehouseModalProps> = ({
  ...
  const {
    ...
    resolver: zodResolver(mode === "create" ? warehouseCreateSchema : warehouseEditSchema) as any,
    defaultValues: {
      name: "",
      address: "",
      countryId: 0,
  ...
  const [originalSectors, setOriginalSectors] = useState<Sector[]>([]);
  ...
  useEffect(() => {
    const initializeForm = async () => {
      ...
      if (warehouse && mode === "edit") {
        ...
        try {
          const response = await warehouseService.getSectorsByWarehouse(warehouse.id);
          const fetchedSectors: Sector[] = (response.content ?? response).map(
            (s: { id: number; name: string }) => ({ id: s.id, name: s.name })
          );
          setSectors(fetchedSectors);
          setOriginalSectors(fetchedSectors);
        } catch (e) {
  ...
  useEffect(() => {
    if (mode === "create") {
      setValue(
        "sectors",
        sectors.map((s) => ({ name: s.name })),
        { shouldValidate: true }
      );
    }
  }, [sectors, setValue, mode]);
  ...
  const computeSectorsDiff = () => {
    ...
    const updated = sectors
      .filter((s) => {
        ...
        const original = originalSectors.find((o) => o.id === s.id);
        return original && original.name !== s.name;
      })
      .map((s) => ({ id: s.id!, name: s.name }));

    const deleted = originalSectors
      .filter((o) => !sectors.find((s) => s.id === o.id))
      .map((o) => o.id!);
  ...
  const onSubmit = async (data: WarehouseFormValues) => {
    ...
    try {
      if (mode === "create") {
        await warehouseService.createWarehouse(data as Warehouse, images);
      } else {
        const { sectors: _, ...warehouseData } = data;
        const diff = computeSectorsDiff();
        const hasSectorChanges =
          diff.added.length > 0 || diff.updated.length > 0 || diff.deleted.length > 0;

        await Promise.all([
          warehouseService.updateWarehouse(warehouse!.id, warehouseData as Warehouse),
          hasSectorChanges
            ? warehouseService.patchSectors(warehouse!.id, diff)
            : Promise.resolve(),
        ]);

        if (imagesToDelete.length > 0) {
          await warehouseService.deleteImages(warehouse!.id, imagesToDelete);
        }

        if (images.length > 0) {
          const formData = new FormData();
          images.forEach((img) => formData.append("images", img));
          await warehouseService.uploadWarehouseImages(warehouse!.id, formData);
        }
      }
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-400 scrollbar-track-gray-100 hover:scrollbar-thumb-indigo-500">
        ...
        <div className="p-6 space-y-6">
          {submitStatus === "success" && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3">
              ...
              <p className="text-green-800">
                Warehouse {mode === "create" ? "created" : "updated"} successfully!
              </p>
            </div>
          )}
          ...
          <div>
            ...
            {sectors.length === 0 && (
              <p className="text-sm text-gray-400 mb-2">No sectors added yet.</p>
            )}

            {mode === "create" && errors.sectors && (
              <p className="mb-2 text-sm text-red-600">
                {(errors.sectors as any).message ?? "At least one sector is required"}
              </p>
            )}

            <div className="space-y-2">
              {sectors.map((sector, index) => (
                <div key={sector.id ?? `new-${index}`} className="flex items-center gap-2">
                  <input
                    type="text"
                    className="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    value={sector.name}
                    onChange={(e) => updateSectorName(index, e.target.value)}
                    placeholder={`Sector ${index + 1} name`}
                  />
                  <button
                    type="button"
          ...
          <div>
            ...
            >
              ...
              <label htmlFor="warehouse-images" className="cursor-pointer">
                ...
                <p className="text-sm text-gray-600">Click to upload warehouse images</p>
